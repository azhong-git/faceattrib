...
class DirectoryIterator(Iterator):
    ...
    # Arguments
        ...
        interpolation: Interpolation method used to resample the image if the
            ...
            supported. If PIL version 3.4.0 or newer is installed, "box" and
            "hamming" are also supported. By default, "nearest" is used.
        multilabel_classes: Dictionary, mapping filepaths to numpy array of
            (multiple) classes (e.g.  {'folder/some_image.jpg':
            np.array([ 0.,  1.,  0.,  1.]),...})
    """
    ...
    def __init__(self, directory, image_data_generator,
        ...
        if(self.class_mode == 'multilabel'):
            ...
            i = 0
            for dirpath in (os.path.join(directory, subdir) for subdir in ['all']):
                for filename in sorted(os.listdir(dirpath)):
                    if filename.endswith('png') or filename.endswith('jpg') or filename.endswith('jpeg') or \
                       filename.endswith('bmp') or filename.endswith('ppm') or filename.endswith('tif') or \
                       filename.endswith('tiff'):
                        self.classes[i] = self.multilabel_classes[filename]
                        i+=1
